...
# Remove all files matching a glob pattern in a target directory
# Usage: remove_roles_by_pattern "/target/dir" "pattern-*"
remove_roles_by_pattern() {
    ...
    if [[ $count -gt 0 ]]; then
        log_info "Cleaned up $count existing commands"
    fi
}
...
# Usage: setup_roles_for_cli "CLI Name" "cli_key" "/target/dir" process_role [cleanup_fn]
#
# Arguments:
...
#   $5 - (Optional) Cleanup callback, called with target_dir before role processing
#
# Callback signature:
...
setup_roles_for_cli() {
    ...
    if ! agent_enabled "$cli_name"; then
        ...
        echo "  Add '$cli_key' to agents list in defaults.yml"
    ...
    # Run cleanup if callback provided (removes stale role files before re-creating)
    if [[ -n "$cleanup_fn" ]]; then
        "$cleanup_fn" "$target_dir"
    fi
    ...
    if [[ -z "$enabled_roles" ]]; then
        ...
        echo "To enable roles, update the roles.enabled list in defaults.yml"
        return 0
    fi

    log_success "Enabled $(echo "$enabled_roles" | wc -w | tr -d ' ') roles"
    echo ""
...
export -f remove_roles_by_pattern
