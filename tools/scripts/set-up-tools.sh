...
export NEO4J_URI="bolt://localhost:$NEO4J_DB_PORT"
...
start_neo4j_docker() {
    ...
    # - port $NEO4J_DB_PORT on host for Bolt protocol (7687 within container)
    # - maps the directory $NEO4J_STORAGE_PATH on host to /data in the container
    docker run -d \
        --name "$container_name" \
        -p "$NEO4J_HTTP_PORT:7474" \
        -p "$NEO4J_DB_PORT:7687" \
        -v "$NEO4J_STORAGE_PATH:/data" \
        -e "NEO4J_AUTH=${NEO4J_USERNAME}/${NEO4J_PASSWORD}" \
        -e "NEO4J_PLUGINS=${NEO4J_PLUGINS}" \
        neo4j:5 >/dev/null
...
# Wait for Neo4j to be ready (checks bolt port)
wait_for_neo4j_ready() {
    log_info "Waiting for Neo4j to be ready..."
    local max_wait=60
    local waited=0
    while ! nc -z localhost "$NEO4J_DB_PORT" 2>/dev/null; do
        sleep 2
        waited=$((waited + 2))
    ...
    log_success "Neo4j ready on bolt://localhost:${NEO4J_DB_PORT}"
}
...
# =============================================================================
# Neo4j Graph Memory MCP configuration
# =============================================================================

# Neo4j memory: mcp-neo4j-memory for entity/relation CRUD
setup_neo4j_memory_mcp() {
    log_info "Neo4j URI: $NEO4J_URI"
    setup_stdio_mcp "neo4j-memory" "env" \
        "NEO4J_URI=$NEO4J_URI" \
...
# Neo4j memory backend: gds-agent for Graph Data Science algorithms
setup_gds_agent_mcp() {
    log_info "Configuring GDS Agent MCP..."
    setup_stdio_mcp "neo4j-gds" "env" \
        "NEO4J_URI=$NEO4J_URI" \
        "NEO4J_USERNAME=$NEO4J_USERNAME" \
        "NEO4J_PASSWORD=$NEO4J_PASSWORD" \
        "uvx" "gds-agent"
...
# Neo4j memory backend: bureau-graph-extras for blast_radius and cycle detection
setup_bureau_graph_extras_mcp() {
    log_info "Configuring Bureau Graph Extras MCP..."
    setup_stdio_mcp "bureau-graph-extras" "env" \
        "NEO4J_URI=$NEO4J_URI" \
        "NEO4J_USERNAME=$NEO4J_USERNAME" \
        "NEO4J_PASSWORD=$NEO4J_PASSWORD" \
        "uvx" "bureau-graph-extras"
}
...
configure_auto_approve() {
    ...
    local mcp_servers=()
    mcp_servers+=("pal" "qdrant" "semgrep" "fs" "fetch" "git" "playwright")

    # Add Neo4j graph memory servers
    mcp_servers+=("neo4j-memory" "neo4j-gds" "bureau-graph-extras")
...
# Neo4j (Docker container for graph memory backend)
log_info "Starting Neo4j Docker container..."
start_neo4j_docker
...
log_separator
log_info "Configuring agents to use Neo4j Graph Memory MCPs..."
setup_neo4j_memory_mcp
setup_gds_agent_mcp
setup_bureau_graph_extras_mcp
